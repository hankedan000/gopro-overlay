...
namespace cv
{
        ...
        const int cornerRadius)
    {
        ...
        // |     |
        // p4 - p3
        const auto & p1 = topLeft;
        const auto   p2 = cv::Point(bottomRight.x, topLeft.y);
        const auto & p3 = bottomRight;
        const auto   p4 = cv::Point(topLeft.x, bottomRight.y);
        
        if (thickness < 0)
        {
            // big rect
            const cv::Point top_left_main_rect(int(p1.x + cornerRadius), int(p1.y));
            const cv::Point bottom_right_main_rect(int(p3.x - cornerRadius), int(p3.y));

            const cv::Point top_left_rect_left(p1.x, p1.y + cornerRadius);
            const cv::Point bottom_right_rect_left(p4.x + cornerRadius, p4.y - cornerRadius);

            const cv::Point top_left_rect_right(p2.x - cornerRadius, p2.y + cornerRadius);
            const cv::Point bottom_right_rect_right(p3.x, p3.y - cornerRadius);
